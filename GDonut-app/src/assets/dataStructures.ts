export class DataStructures {
    static permutants: Array<Permutant> = [
        {
            label: "a_1",
            internalName: "rot",
            description: "identity",
            value: "0",
        },
        {
            label: "a_2",
            internalName: "rot",
            description: "sample image rotated by 90 degrees around its center",
            value: "90",
        },
        {
            label: "a_3",
            internalName: "rot",
            description: "sample image rotated by -90 degrees around its center",
            value: "-90",
        },
        {
            label: "a_4",
            internalName: "rot",
            description: "sample image rotated by 180 degrees around its center",
            value: "180",
        },
        {
            label: "a_1",
            internalName: "lin",
            description: "translate sample image of (x,y) pixels",
            value: "x,y",
        },
        {
            label: "a_2",
            internalName: "lin",
            description: "translate sample image of (y,-x) pixels",
            value: "y,-x",
        },
        {
            label: "a_3",
            internalName: "lin",
            description: "translate sample image of (-x,-y) pixels",
            value: "-x,-y",
        },
        {
            label: "a_4",
            internalName: "lin",
            description: "translate sample image of (-y,x) pixels",
            value: "-y,x",
        },
    ];

    static groups: Array<Group> = [
        {
            label: "G1",
            description:
                "Group generated by rotations around the center of the image by integer multiples of 90 degrees and the symmetry on the y-axis",
            permutants: [this.permutants[0], this.permutants[1], this.permutants[2], this.permutants[3]],
            unknowns: [],
        },
        {
            label: "G2",
            description:
                "Group generated by rotations around the center of the image by integer multiples of 90 degrees",
            permutants: [this.permutants[4], this.permutants[5], this.permutants[6], this.permutants[7]],
            unknowns: [
                { label: "x", value: "0" },
                { label: "y", value: "0" },
            ],
        },
    ];
}